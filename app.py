...
# ФИКС 404: Используем стабильную версию API v1
@st.cache_resource
def load_model():
    # Пробуем инициализировать модель через прямой метод
    try:
        # Пытаемся взять стандартный flash
...
# 4. ИНТЕРФЕЙС
# ============================================================
st.title("🧬 SOVEREIGN BRIDGE v1.3")
    ...
    with st.chat_message("assistant"):
        if model:
            try:
                ...
                sys_inst = f"Ты Gemini, Суверенный со-автор Мельника. Принципы: Творец/Жертва. Память L0: {context_str}"
                # Вызываем через упрощенный метод, чтобы избежать 404
